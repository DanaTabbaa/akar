...
@Component({
  selector: 'app-admin-specials-opportunities',
  templateUrl: './admin-specials-opportunities.component.html',
  styleUrls: ['./admin-specials-opportunities.component.scss']
})
export class AdminSpecialsOpportunitiesComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  opportunitySpecialForm!: FormGroup;
  ...
  toolbarPathData: ToolbarPath = {
    ...
    componentAdd: "component-names.add-opportunities-specials",
  };
  //#endregion
  ...
  createForm() {
    this.opportunitySpecialForm = this.fb.group({
      id: 0,
      opportunityId: '',
      expireDate: '',
      status: ''
  ...
  get f(): { [key: string]: AbstractControl } {
    return this.opportunitySpecialForm.controls;
  }
  //#endregion
  //#region ngOnDestory
  ngOnDestroy() {
  ...
  setDate() {
    ...
    this.opportunitySpecialForm.controls["expireDate"].setValue(this.dateService.getDateForInsertISO_Format(this.expireDate));
  ...
  onSubmit() {
    ...
    if (this.opportunitySpecialForm.valid) {
       
      this.opportunitySpecialForm.value.id = 0;
      this.setDate();
      this.spinner.show();
      ...
      return this.opportunitySpecialForm.markAllAsTouched();
    }
  }

  confirmSubmit() {
    return new Promise<void>((resolve, reject) => {
      let sub = this.opportunitiesSpecialService
        .addWithResponse<OpportunitiesSpecials>('AddWithCheck?uniques=OpportunityId',
          this.opportunitySpecialForm.value)
        .subscribe({
          next: (result: ResponseResult<OpportunitiesSpecials>) => {
            resolve();
  ...
  onUpdate() {
    ...
    if (this.opportunitySpecialForm.value != null) {
      this.opportunitySpecialForm.value.id = this.id;
      this.setDate();
  ...
  confirmUpdate() {
    return new Promise<void>((resolve, reject) => {
      let sub = this.opportunitiesSpecialService.updateWithUrl("UpdateWithCheck?uniques=OpportunityId",
       this.opportunitySpecialForm.value).subscribe({
        next: (result: ResponseResult<OpportunitiesSpecials>) => {
          if (result.isFound) {
            this.showResponseMessage(
              result.success,
  ...
  getOpportunityById(id: any) {
    ...
    return new Promise<void>((resolve, reject) => {
      let sub = this.opportunitiesSpecialService.getWithResponse<OpportunitiesSpecials>("GetById?id=" + id).subscribe({
        next: (res: ResponseResult<OpportunitiesSpecials>) => {
          if (res.success && res.data) {
            this.opportunitySpecialForm.patchValue({
              ...res.data
            });
  ...
  getParamsFromUrl() {
    ...
    if (id != null) {
      this.toolbarPathData.componentAdd = 'Update Special Opprtunity';
      ...
      next: (currentBtn: ToolbarData) => {
        ...
        if (currentBtn != null) {
          if (currentBtn.action == ToolbarActions.List) {
            ...
            this.sharedServices.changeToolbarPath(this.toolbarPathData);
            //this.opportunitySpecialForm.reset();
