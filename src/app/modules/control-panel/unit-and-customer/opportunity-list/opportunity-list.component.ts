...
export class OpportunityListComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.spinner.show();
    Promise.all([this.managerService.loadPagePermissions(PAGEID), this.managerService.loadOpportunities()]).then(a => {
  ...
  edit(id: string) {
    ...
    this.router.navigate(['/control-panel/definitions/update-opportunity', id]);
  }
  showConfirmDeleteMessage(id: number) {
    ...
    modalRef.result.then(rs => {
      if (rs == "Confirm") {
        ...
        let sub = this.opportunityService.deleteWithUrl("DeleteWithCheck?Id=" + id).subscribe({
          next: (response: ResponseResultNoraml) => {

            if (response.success == true) {
              this.managerService.loadOpportunities().then(a => {
                this.opportunities = this.managerService.getOpportunities();
                this.spinner.hide();
              }).catch(err => {
                this.spinner.hide();
  ...
  defineGridColumn() {
    let sub = this.sharedServices.getLanguage().subscribe(res => {
      ...
      this.columnNames = [
        ...
        {
          title: this.lang == 'ar' ? 'المدة' : ' Period',
          field: 'periodPerDay',
        },
        {
