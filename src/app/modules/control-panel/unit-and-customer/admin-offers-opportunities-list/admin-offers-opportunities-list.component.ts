...
import { OpportunitiesOffersVM } from 'src/app/core/models/ViewModel/opportunities-offers-vm';
import { OpportunitiesOffersService } from 'src/app/core/services/backend-services/opportunities-offers.service';
const PAGEID = 1014;

@Component({
  selector: 'app-admin-offers-opportunities-list',
  templateUrl: './admin-offers-opportunities-list.component.html',
  styleUrls: ['./admin-offers-opportunities-list.component.scss']
})
export class AdminOffersOpportunitiesListComponent implements OnInit, OnDestroy {
  ...
  opportunitiesOffers: OpportunitiesOffersVM[] = [];
  ...
  ngOnInit(): void {
    ...
    this.spinner.show();
    Promise.all([this.managerService.loadPagePermissions(PAGEID), this.managerService.loadOpportunitiesOffers()])
      .then(a => {
        this.opportunitiesOffers = this.managerService.getOpportunitiesOffers();     
        this.spinner.hide();
  ...
  goToAdd(typeOfComponent: any) {
    this.router.navigate(['/control-panel/definitions/add-opportunities-offers'], {
      queryParams: { typeOfComponent: typeOfComponent },
    });
  }
  edit(id: string) {
    this.sharedServices.changeButton({ action: 'Update' } as ToolbarData);
    this.router.navigate(['/control-panel/definitions/update-opportunities-offers', id]);
  }
  showConfirmDeleteMessage(id: number) {
    ...
    modalRef.result.then((rs) => {
      if (rs == 'Confirm') {
        ...
        let sub = this.opportunitiesOfferService.deleteWithUrl("DeleteWithCheck?id=" + id).subscribe(
          {
            next:(resonse:ResponseResultNoraml) => {
              if (resonse.success == true) {
                this.managerService.loadOpportunitiesOffers().then(a => {
                  this.spinner.hide();
                  this.opportunitiesOffers = this.managerService.getOpportunitiesOffers();              
                }).catch(e => {
                  this.spinner.hide();
    ...
    let sub = this.sharedServices.getLanguage().subscribe(res => {
      ...
      this.columnNames = [
       ...
       
        {
          ...
          field: 'offerExpireDate',
        },
      
        this.lang == "ar" ? {
          title: "حذف",
          field: "", formatter: this.deleteFormatIcon, cellClick: (e, cell) => {
            this.delete(cell.getRow().getData().offerId);
          },
        } :
          {
            title: "Delete",
        ...
        this.lang == "ar" ? {
          ...
          field: "", formatter: this.editFormatIcon, cellClick: (e, cell) => {
            this.edit(cell.getRow().getData().offerId);
          }
        }
          :
          {
