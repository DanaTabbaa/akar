...
export class DraggableMenuItemsComponent implements AfterViewInit, OnInit, OnChanges {
  ...
  addDragEvents() {
    ...
    draggableItems.forEach(draggable => {
      ...
      draggable.addEventListener('dragend', (e) => {
        ...
        ////(("drag stopped")
        //
        // //(("Current Menu Item", self.currentMenuItem);
        // //(("After menus item", self.afterMenuItem)
  ...
  ngOnChanges() {
    if (this.menuItems.length) {
      if (this.enableSort) {
        //
        ////(("sort by from mmeu Items", this.sortBy);
