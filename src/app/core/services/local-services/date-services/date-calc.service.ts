...
export class DateCalculation {
  ...
  calculateEndDateForHijri(period: number, rentMethodType: number,
    ...
    ////(("final Hijridate", hijriStartDate);
    if (hijriStartDate.day - daysBefore == 0) {
      ...
      ////(("Hijridate after Minus Day and month");
    }
    else {
      // //(("Hijri Day Before Minus 1 ", hijriStartDate.day)
    ...
    // //(("Before Convert to Hijri ##################################33 ", hijriStartDate);
  ...
  calculateEndDateForGregrion(period: number, rentMethodType: number, startDate: DateModel, daysBefore: number): DateModel {
    ...
    if (rentMethodType == 1) {
      ...
      addedYears = Math.floor(period / 12);
      addedMonth = period % 12;
      //(("Period Division", Math.floor(period / 12), period % 12)
  ...
  calculateEndDateForGregrionByPeriod(period: number, startDate: DateModel, daysBefore: number): DateModel {
    let endDate: { year: number, month: number, day: number } = {
      day: 0,
      month: 0,
      year: 0
  ...
  getHiriDate(georginDate:Date){
    return this.ngbCalendar.fromGregorian(georginDate);
  }
