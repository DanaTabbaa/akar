...
import { ResponseData, ResponseResult, ResponseResultNoraml } from '../../models/ResponseResult';
...
export class BaseService<T>
{
    ...
    addWithResponse<G>(url:string , model: G) {
        ...
        let attachmentIds = localStorage.getItem("attachments")??0;
        if(url.includes("?"))
        {
          apiUrl +="&ids="+ attachmentIds;
        }
        else{
    ...
    getWithResponseNormal(url:string ) {
        this.apiHeaders = this.apiHeaders.set('Authorization', 'Bearer ' + localStorage.getItem('token'));
        return this.http.get<ResponseResultNoraml>(this.getFullURL(url), { headers: this.apiHeaders });
    }


    post(url:string, data:any ) {
        this.apiHeaders = this.apiHeaders.set('Authorization', 'Bearer ' + localStorage.getItem('token'));
        return this.http.post<ResponseResultNoraml>(this.getFullURL(url),data, { headers: this.apiHeaders });
    }

    getWithResponseForWebsite<M>(url:string ) {
        //this.apiHeaders = this.apiHeaders.set('Authorization', 'Bearer ' + localStorage.getItem('token'));
        return this.http.get<ResponseResult<M>>(this.getFullURL(url));
    }

    postWithResponseForWebsite<M>(url:string , model:any) {
        //this.apiHeaders = this.apiHeaders.set('Authorization', 'Bearer ' + localStorage.getItem('token'));
        return this.http.post<ResponseResult<M>>(this.getFullURL(url), model);
    }
    ...
    uploadWithUrl(url: string, model: any) {
      ;
      this.apiHeaders = this.apiHeaders.set('Authorization', 'Bearer ' + localStorage.getItem('token'));
      return this.http.post<any>(this.getFullURL(url), model, { headers: this.apiHeaders, reportProgress: true,
        responseType: 'json', });
  }
